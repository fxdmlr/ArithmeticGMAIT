...
def rootgame(inpt_dict):
    inp_ndig = inpt_dict['ndigits']
    out_ndig = inpt_dict['resdig']
    n = inpt_dict['n']
    num = random.randint(10 ** (inp_ndig - 1), 10 ** (inp_ndig) - 1) 
    if n == 2:
        string = '  ' + ''.join(['_' for i in str(num)]) + '\n\\/'+str(num) + ' = '
    elif n > 2:
        string = str(n) + ''.join(['_' for i in str(num)]) + '\n' + ''.join([" " for i in str(n)])+ '\\/'+str(num) + ' = '
    r = num ** (1 / n)
    res = int(r * 10 ** out_ndig) / 10 ** out_ndig
